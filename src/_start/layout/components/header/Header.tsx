...
export function Header() {
  ...
  const { pageTitle, pageTitleSub, moduleName } = usePageData();
  return (
    <>
      <div
        className={`header-wrapper-container d-flex align-items-stretch justify-content-between`}
      ...
      >
        ...
        <div className="d-flex align-items-center">
          {config.aside.content && config.aside.content === "menu" && (
            <div>
            <h3 className="text-light custom-header-title mt-10">{pageTitle}</h3>
            <h3 className="custom-header-title-sub ">{pageTitleSub}</h3>
            </div>
          )}
